...
function search($tcodeid, $query){
	...
	// HR First
	if($query == 'hr_first'){
		$request = "SELECT * FROM reporting_date_info WHERE type_reporting = 2 AND trans_code = '$tcodeid' AND request_status = 1";
		$request_data = mysql_query($request);
	}//end if
	
	// Closed
	if($query == 'closed'){
		$request = "SELECT * FROM reporting_date_info WHERE type_reporting = 2 AND trans_code = '$tcodeid' AND request_status = 2";
		$request_data = mysql_query($request);
	}//end if
