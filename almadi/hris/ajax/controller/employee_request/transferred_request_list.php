...
// HR First
if($_GET['request_type'] == 'transferred_hr_first'){
	$joinQuery = "FROM `reporting_date_info` AS `rdi` JOIN `employee_details` AS `ed` ON (`ed`.`id_number` = `rdi`.`emp_id_number`)";
	$extraWhere = "`ed`.`employment_status` = 1 AND `rdi`.`type_reporting` = 1 AND `rdi`.`request_status` = 1 AND `ed`.`project_id` IN ($projects)";
}// end if

// Closed
if($_GET['request_type'] == 'transferred_closed'){
	$joinQuery = "FROM `reporting_date_info` AS `rdi` JOIN `employee_details` AS `ed` ON (`ed`.`id_number` = `rdi`.`emp_id_number`)";
	$extraWhere = "`ed`.`employment_status` = 1 AND `rdi`.`type_reporting` = 1 AND `rdi`.`request_status` = 2 AND `ed`.`project_id` IN ($projects)";
}// end if
