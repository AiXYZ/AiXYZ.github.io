...
function search($tcodeid){
	...
	$vacation = "SELECT * FROM vacation_request_info WHERE tcode_vacation = '$tcodeid' AND hr_update_by IS NOT NULL AND fd_update_by IS NOT NULL AND hr_final_update_by IS NOT NULL AND tcode_vacation IN (SELECT tcode FROM process_visa) AND tcode_vacation NOT IN (SELECT tcode FROM process_ticket) AND ISNULL(decline_by)";
...
// 	//HR Final Details
	...
	return array(
			...
			'encoder_id_ticket' => $encoder_id,
			'encoder_name_ticket' => $encoder_name,
			'encoder_created_ticket' => $encoder_created,
			'elapsed_days_ticket' => $days_between,
			
			'employee_id_ticket' => $employee_id,
			'employee_name_ticket' => $employee_name,
			'employee_pname_ticket' => $employee_pname,
			
			'employee_position_ticket' => $employee_position,
			'employee_leave_type_ticket' => $employee_leave_type,
			'employee_number_of_days_ticket' => $employee_number_of_days,
			
			'expected_departure_from_date_ticket' => $expected_departure_from_date,
			'expected_departure_to_date_ticket' => $expected_departure_to_date,
			'who_will_pay_visa_TICKET' => $who_will_pay_visa,
			
			'replacement_requirements_id_ticket' => $replacement_requirements_id,
			
			'replacement_requirements_ticket' => $replacement_requirements,
			'vacation_request_file_path_ticket' => $vacation_request_file_path,
			'vacation_request_file_ticket' => $vacation_request_file,
			'employee_iqama_number_ticket' => $employee_info_iqama_number,
			
			'employee_info_contract_g_date_employed_ticket' => $employee_info_contract_g_date_employed,
			'employee_info_contract_years_ticket' => $employee_info_contract_years,
			'employee_length_of_service_ticket' => $length_of_service,
			
			'employee_last_vacation_date_ticket' => $employee_last_vacation_date,
			'employee_date_joined_last_vacation_ticket' => $employee_date_joined_last_vacation,
			'employee_days_of_last_vacation_ticket' => $employee_days_of_last_vacation,
			
			'employee_ltime_after_last_vacation_ticket' => $employee_length_of_service,
			'employee_vr_summary_ticket' => $employee_vr_summary,
			'reason_for_vacation_ticket' => $reason_for_vacation,
			
			'required_replacement_ticket' => $replacement_requirements,
			'replacement_id_number_ticket' => $replacement_id_number,
			'replacement_name_ticket' => $replacement_name,
			
// 			'ticket_hr_first_employee_id' => $ticket_hr_first_employee_id,
// 			'ticket_hr_first_employee_name' => $ticket_hr_first_name,
// 			'ticket_hr_first_created' => $ticket_hr_first_created,
// 			'ticket_hr_first_update_days' => $ticket_hr_first_update_days,
// 			'ticket_hr_first_file_path' => $ticket_hr_first_file_path,
// 			'ticket_hr_first_file_name' => $ticket_hr_first_file_name,
// 			'ticket_hr_first_notes' => $ticket_hr_first_notes,
			
// 			'ticket_fd_encoder_id_fd' => $encoder_id_fd,
// 			'ticket_fd_encoder_name_fd' => $encoder_name_fd,
// 			'ticket_fd_encoder_created_fd' => $encoder_created_fd,
			
// 			'ticket_fd_encoder_days' => $encoder_days_fd,
// 			'ticket_fd_basic_salary' => $fd_basic_salary,
// 			'ticket_fd_total_deductions' => $fd_total_deductions,
			
// 			'ticket_fd_credit' => $fd_credit,
// 			'ticket_fd_file_path' => $fd_file_path,
// 			'ticket_fd_file_name' => $fd_file_name,
			
// 			'ticket_fd_notes' => $fd_notes,
			
// 			'ticket_hr_final_update_by' => $hr_final_update_by,
// 			'ticket_hr_final_encoder_name' => $encoder_name_hr_final,
// 			'ticket_hr_final_update_created' => $hr_final_update_created,
			
// 			'ticket_hr_final_days' => $hr_final_days,
// 			'ticket_hr_final_file_path' => $hr_final_file_path,
// 			'ticket_hr_final_file_name' => $hr_final_file_name,
			
// 			'ticket_hr_final_notes' => $hr_final_notes
