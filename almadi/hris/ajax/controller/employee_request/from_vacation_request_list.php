...
// HR received
if($_GET['request_type'] == 'FromV_hr'){
	$joinQuery = "FROM `reporting_date_info` AS `u` JOIN `employee_details` AS `ud` ON (`ud`.`id_number` = `u`.`emp_id_number`)";
	//$extraWhere = "`ud`.`employment_status` = 1 AND `u`.`request_status` = 1 AND `ud`.`project_id` IN ($projects)";
	$extraWhere = "`ud`.`employment_status` = 1 AND `u`.`type_reporting` = 3 AND `u`.`request_status` = 1 AND `ud`.`project_id` IN ($projects)"; // Change 07 21 2016 GBL, type_reporting is 3
}// end if

// hr closing
if($_GET['request_type'] == 'FromV_closed'){
	$joinQuery = "FROM `reporting_date_info` AS `u` JOIN `employee_details` AS `ud` ON (`ud`.`id_number` = `u`.`emp_id_number`)";
	//$extraWhere = "`ud`.`employment_status` = 1 AND `u`.`request_status` = 2 AND `ud`.`project_id` IN ($projects)";
	$extraWhere = "`ud`.`employment_status` = 1 AND `u`.`type_reporting` = 3 AND `u`.`request_status` = 2 AND `ud`.`project_id` IN ($projects)"; // Change 07 21 2016 GBL, type_reporting is 3
}// end if
