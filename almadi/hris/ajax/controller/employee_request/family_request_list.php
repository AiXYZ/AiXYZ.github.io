...
// HR received
if($_GET['request_type'] == 'fv_hr'){
	$joinQuery = "FROM `family_info` AS `u` JOIN `employee_details` AS `ud` ON (`ud`.`id_number` = `u`.`emp_id_number`)";
	$extraWhere = "`ud`.`employment_status` = 1 AND `u`.`request_status` = 1 AND `ud`.`project_id` IN ($projects)";
}// end if

// hr closing
if($_GET['request_type'] == 'fv_closed'){
	$joinQuery = "FROM `family_info` AS `u` JOIN `employee_details` AS `ud` ON (`ud`.`id_number` = `u`.`emp_id_number`)";
	$extraWhere = "`ud`.`employment_status` = 1 AND `u`.`request_status` = 2 AND `ud`.`project_id` IN ($projects)";
}// end if

// closed
if($_GET['request_type'] == 'fv_declined'){
	$joinQuery = "FROM `family_info` AS `u` JOIN `employee_details` AS `ud` ON (`ud`.`id_number` = `u`.`emp_id_number`)";
