...
function search($tcodeid){
	...
	// HR First Details
	@$hr_final_employee_id = mysql_real_escape_string(mysql_result($vacation_data,0,"hr_update_by"));
	@$hr_final_created = mysql_real_escape_string(mysql_result($vacation_data,0,"hr_update_created"));
	@$hr_update_days = mysql_real_escape_string(mysql_result($vacation_data,0,"hr_update_days"));
	@$hr_final_file_path = mysql_real_escape_string(mysql_result($vacation_data,0,"vr_request_file_initial"));
	@$hr_file_path_explode = explode("|",separateFilePathOnMultipleFolders($hr_final_file_path));
	@$hr_final_file_name = $hr_file_path_explode[0];
	@$hr_final_notes = mysql_real_escape_string(mysql_result($vacation_data,0,"notes_hr"));
	
	// Name of the hr first update
	$encoder = "SELECT id_number, fullname FROM employee_details WHERE id_number = '".$hr_final_employee_id."' ";
	$encoder_data = mysql_query($encoder);
	@$hr_final_name = mysql_result($encoder_data,0,"fullname");
	// Name of the hr first update
	// HR First Details
	...
	$encoder_fd = "SELECT id_number, fullname FROM employee_details WHERE id_number = '".$hr_final_employee_id."' ";
	...
	return array(
			'tcode_hr_final' => $tcode,
			
			'encoder_id_hr_final' => $encoder_id,
			'encoder_created_hr_final' => $encoder_created,
			'encoder_name_hr_final' => $encoder_name,
			'elapsed_days_hr_final' => $days_between,
			
			'employee_id_hr_final' => $employee_id,
			'employee_name_hr_final' => $employee_name,
			'employee_pname_hr_final' => $employee_pname,
			
			'employee_position_hr_final' => $employee_position,
			'employee_leave_type_hr_final' => $employee_leave_type,
			'employee_number_of_days_hr_final' => $employee_number_of_days,
			
			'expected_departure_from_date_hr_final' => $expected_departure_from_date,
			'expected_departure_to_date_hr_final' => $expected_departure_to_date,
			'who_will_pay_visa_hr_final' => $who_will_pay_visa,
			
			'replacement_requirements_id_hr_final' => $replacement_requirements_id,
			
			'replacement_requirements_hr_final' => $replacement_requirements,
			'vacation_request_file_path_hr_final' => $vacation_request_file_path,
			'vacation_request_file_hr_final' => $vacation_request_file,
			'employee_iqama_number_hr_final' => $employee_info_iqama_number,
			
			'employee_info_contract_g_date_employed_hr_final' => $employee_info_contract_g_date_employed,
			'employee_info_contract_years_hr_final' => $employee_info_contract_years,
			'employee_length_of_service_hr_final' => $length_of_service,
			
			'employee_last_vacation_date_hr_final' => $employee_last_vacation_date,
			'employee_date_joined_last_vacation_hr_final' => $employee_date_joined_last_vacation,
			'employee_days_of_last_vacation_hr_final' => $employee_days_of_last_vacation,
			
			'employee_length_of_service_hr_final' => $employee_length_of_service,
			'employee_vr_summary_hr_final' => $employee_vr_summary,
			'reason_for_vacation_hr_final' => $reason_for_vacation,
			
			'replacement_id_number_hr_final' => $replacement_id_number,
			'replacement_name_hr_final' => $replacement_name,
			
			'hr_final_employee_id' => $hr_final_employee_id,
			'hr_final_name' => $hr_final_name,
			'hr_final_created' => $hr_final_created,
			'hr_final_update_days' => $hr_update_days,
			'hr_final_file_path' => $hr_final_file_path,
			'hr_final_file_name' => $hr_final_file_name,
			'hr_final_notes' => $hr_final_notes,
			
			'encoder_id_fd' => $encoder_id_fd,
			'encoder_name_fd' => $encoder_name_fd,
			'encoder_created_fd' => $encoder_created_fd,
			
			'encoder_days_fd' => $encoder_days_fd,
			'fd_basic_salary' => $fd_basic_salary,
			'fd_total_deductions' => $fd_total_deductions,
			
			'fd_credit' => $fd_credit,
			'fd_file_path' => $fd_file_path,
			'fd_file_name' => $fd_file_name,
