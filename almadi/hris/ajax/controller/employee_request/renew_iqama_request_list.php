...
$columns = array(
		...
		array( 'db' => '`rii`.`tcode`', 'dt' => 7, 'field' => 'tcode' ),
...
// HR First
if($_GET['request_type'] == 'renew_iqama_hr_first'){
	$joinQuery = "FROM `renew_iqama_info` AS `rii` JOIN `employee_details` AS `ed` ON (`ed`.`id_number` = `rii`.`emp_id_number`)";
	$extraWhere = "`ed`.`employment_status` = 1 AND `rii`.`iqama_status` = 1 AND `ed`.`project_id` IN ($projects)";
}// end if

// Closed
if($_GET['request_type'] == 'renew_iqama_closed'){
	$joinQuery = "FROM `renew_iqama_info` AS `rii` JOIN `employee_details` AS `ed` ON (`ed`.`id_number` = `rii`.`emp_id_number`)";
	$extraWhere = "`ed`.`employment_status` = 1 AND `rii`.`iqama_status` = 2 AND `ed`.`project_id` IN ($projects)";
}// end if
