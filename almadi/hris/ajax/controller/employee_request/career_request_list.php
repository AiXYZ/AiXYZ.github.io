...
// HR received
if($_GET['request_type'] == 'cr_hr'){
	$joinQuery = "FROM `career_info` AS `u` JOIN `employee_details` AS `ud` ON (`ud`.`id_number` = `u`.`emp_id_number`)";
	$extraWhere = "`ud`.`employment_status` = 1 AND `u`.`request_status` = 1 AND `ud`.`project_id` IN ($projects)";
}// end if

// hr closing
if($_GET['request_type'] == 'cr_closed'){
	$joinQuery = "FROM `career_info` AS `u` JOIN `employee_details` AS `ud` ON (`ud`.`id_number` = `u`.`emp_id_number`)";
	$extraWhere = "`ud`.`employment_status` = 1 AND `u`.`request_status` = 2 AND `ud`.`project_id` IN ($projects)";
}// end if

// closed
if($_GET['request_type'] == 'cr_declined'){
	$joinQuery = "FROM `career_info` AS `u` JOIN `employee_details` AS `ud` ON (`ud`.`id_number` = `u`.`emp_id_number`)";
