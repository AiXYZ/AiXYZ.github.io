...
// 	$typeDepartment = $department['hr_department']; // <-- 1 - HR, 2 - FD, 3 - All Dept, 4 - Project Site, 5 - PM, 6 - AM
	...
	$crRequest_HR = "SELECT cri.emp_id_number, cri.request_status, ed.project_id, ed.id_number, ed.employment_status FROM family_info as cri JOIN employee_details as ed ON (ed.id_number = cri.emp_id_number) where ed.employment_status = 1 and cri.request_status = 1 AND ed.project_id IN ($projects)";
	...
	$crRequest_HR_Closed = "SELECT cri.emp_id_number, cri.request_status, ed.project_id, ed.id_number, ed.employment_status FROM family_info as cri JOIN employee_details as ed ON (ed.id_number = cri.emp_id_number) where ed.employment_status = 1 and cri.request_status = 2 AND ed.project_id IN ($projects)";
	...
	$crRequest_HR_Declined = "SELECT cri.emp_id_number, cri.request_status, ed.project_id, ed.id_number, ed.employment_status FROM family_info as cri JOIN employee_details as ed ON (ed.id_number = cri.emp_id_number) where ed.employment_status = 1 and cri.request_status = 3 AND ed.project_id IN ($projects)";
	...
	return array(
			'fv_Hr_total' => '('.$cr_HR_total.')',
			'fv_Closed_total' => '('.$cr_HR_Closed_total.')',
			'fv_Declined_total' => '('.$cr_HR_Declined_total.')',
			'fv_Hr_total_All' => '('.$cr_HR_total.')',
	);// end array
